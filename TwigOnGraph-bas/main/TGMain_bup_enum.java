...
import queryEvaluator.TG_bup_enum;


public class TGMain_bup_enum {
	...
	public TGMain_bup_enum(String dataFN, String queryFN) {
	...
	private void evaluate() {
		for (int i = 0; i < Flags.REPEATS; i++) {
			for (int Q = 0; Q < queries.size(); Q++) {
				...
				System.out.println("\nEvaluating query " + Q + " ...");
				TG_bup_enum tld = new TG_bup_enum(query, invLsts, invLstsByID, bfl);
	...
	public static void main(String[] args) {
		...
		TGMain_bup_enum psMain = new TGMain_bup_enum(dataFileN, queryFileN);
		psMain.run();
