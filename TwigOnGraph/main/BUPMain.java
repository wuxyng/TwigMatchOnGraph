...
public class BUPMain {
	...
	public BUPMain(String dataFN, String queryFN) {
		...
		// outFileN = Consts.OUTDIR_TREE + "sum_" + fn + "Tree_BUP" + suffix;
		outFileN = Consts.OUTDIR + "sum_" + fn + "Tree_BUP" + suffix;
		stats = new QueryEvalStats(dataFileN, queryFileN, "TreeEval_BUP");
	...
	private void evaluate() {
		...
		for (int i = 0; i < Flags.REPEATS; i++) {
			for (int Q = 0; Q < queries.size(); Q++) {
				...
				// RootDagHomBUPFlt eva = new RootDagHomBUPFlt(query, fb, bfl);
				BUP eva = new BUP(query, fb, bfl, invLsts);
				...
				try {
					...
					stats.add(i, Q, stat);
					// e.printStackTrace();
					System.err.println("Unable to handlet!");
	...
	public static void main(String[] args) {
		...
		BUPMain demain = new BUPMain(dataFileN, queryFileN);
