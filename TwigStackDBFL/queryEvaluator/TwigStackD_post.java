...
public class TwigStackD_post {
	...
	public TwigStackD_post(TreeQuery query, ArrayList<ArrayList<GraphNode>> invLsts, BFLIndex bfl) {
	...
	public QueryEvalStat run() {
		...
		if (success) {
			...
			twigStackD();
			matm = tt.Stop() / 1000;
			System.out.println("Time on TwigStackD_BFL_post:" + matm + "sec.");
			tt.Start();
			// showSolutions();
		...
		QueryEvalStat stat = new QueryEvalStat(matm, entm, 0, 0);

		return stat;
	}

	public QueryEvalStat runmj() {
			...
			for (QNode qi : children) {
				...
				if (!found) {
					...
					while (qiCur.getCurrent().L_interval.mEnd <= tr.L_interval.mEnd && !found) {
						...
						if (mBFL.reach(tq, tqi) == 1 && !checkMissingDEs(qi, tqi))
							found = true;
						else
							qiCur.advance();
					}
	...
	private boolean sweepPartialSolutions(QNode q, GraphNode tq, HashSet<Integer> missings, Stack<StackEntryMJ> sAct,
		...
		for (QNode qi : children) {
			...
			for (PoolEntryMJ hp : pqi.elist()) {
				...
				// if ((axis == 0 && tq.findOUT(tp.id)) || (axis == 1 &&
				// mBFL.reach(tq, hp.getValue()) == 1)) {
