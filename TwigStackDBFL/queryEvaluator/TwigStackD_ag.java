...
public class TwigStackD_ag {
	...
	public TwigStackD_ag(TreeQuery query, ArrayList<ArrayList<GraphNode>> invLsts, BFLIndex bfl) {
	...
	public QueryEvalStat run() {
		...
		twigStackD();
		matm =  tt.Stop() / 1000; 
		System.out.println("Time on TwigStackD_bfl_ag:" + matm + "sec.");
		tt.Start();
		//printSolutions();
		calTotSolns();
		entm =  tt.Stop() / 1000; 
		...
		if (!q.isSink()) {
			...
			for (QNode qi : children) {
				...
				for (PoolEntry h : pool.elist()) {
					...
					if ((axis == 0 && tq.searchOUT(h.getValue().id)) || (axis == 1 && mBFL.reach(tq, h.getValue()) == 1)) {
						found = true;
						break;
					}
				}
				if (!found) {
					...
					//while (qiCur.getCurrent().L_interval.mEnd <= tq.L_interval.mEnd && !found) {
