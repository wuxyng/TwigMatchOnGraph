 ...
 *
 */
public class TwigStackDMain_ag {
	...
	public TwigStackDMain_ag(String dataFN, String queryFN) {

		queryFileN = Consts.INDIR + queryFN;
		dataFileN = Consts.INDIR + dataFN;
		String suffix = ".csv";
		String fn = queryFN.substring(0, queryFN.lastIndexOf('.'));
		outFileN = Consts.OUTDIR + "sum_"+ fn+"_bflag"+suffix;
		stats = new QueryEvalStats(dataFileN, queryFileN, "TwigStackD-bfl-ag");
	}
	...
	private void evaluate() {
		for (int i = 0; i < Flags.REPEATS; i++) {
			for (int Q = 0; Q < queries.size(); Q++) {
				...
				System.out.println("\nEvaluating query " + Q + " ...");
				TwigStackD_ag tsd = new TwigStackD_ag(query, invLsts, bfl);
	...
	public static void main(String[] args) {
		...
		TwigStackDMain_ag psMain = new TwigStackDMain_ag(dataFileN, queryFileN);
		psMain.run();
	}
