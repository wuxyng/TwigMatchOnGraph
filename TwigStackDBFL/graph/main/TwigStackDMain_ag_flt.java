...
import queryEvaluator.TwigStackD_ag_flt;
 ...
 *
 */
public class TwigStackDMain_ag_flt {
	...
	public TwigStackDMain_ag_flt(String dataFN, String queryFN) {

		queryFileN = Consts.INDIR + queryFN;
		dataFileN = Consts.INDIR + dataFN;
		String suffix = ".csv";
		String fn = queryFN.substring(0, queryFN.lastIndexOf('.'));
		outFileN = Consts.OUTDIR + "sum_"+ fn+"_bflagflt"+suffix;
		stats = new QueryEvalStats(dataFileN, queryFileN, "TwigStackD-bfl-ag-flt");
	}
	...
	private void evaluate() {
		for (int i = 0; i < Flags.REPEATS; i++) {
			for (int Q = 0; Q < queries_tree.size(); Q++) {
				...
				TwigStackD_ag_flt tsd = new TwigStackD_ag_flt(query_tree, invLsts_reduced, bfl);
	...
	public static void main(String[] args) {
		...
		TwigStackDMain_ag_flt psMain = new TwigStackDMain_ag_flt(dataFileN, queryFileN);
		psMain.run();
	}
