...
import queryEvaluator.TwigStackD_post_flt;
 ...
 *
 */
public class TwigStackDMain_post_flt {
	...
	public TwigStackDMain_post_flt(String dataFN, String queryFN) {

		queryFileN = Consts.INDIR + queryFN;
		dataFileN = Consts.INDIR + dataFN;
		String suffix = ".csv";
		String fn = queryFN.substring(0, queryFN.lastIndexOf('.'));
		outFileN = Consts.OUTDIR + "sum_"+ fn+"_bflpostflt"+suffix;
		stats = new QueryEvalStats(dataFileN, queryFileN, "TwigStackD-bfl-post-flt");
	}
	...
	private void evaluate() {
		for (int i = 0; i < Flags.REPEATS; i++) {
			for (int Q = 0; Q < queries_tree.size(); Q++) {
				...
				TwigStackD_post_flt tsd = new TwigStackD_post_flt(query_tree, invLsts_reduced, bfl);
	...
	public static void main(String[] args) {
		...
		TwigStackDMain_post_flt psMain = new TwigStackDMain_post_flt(dataFileN, queryFileN);
		psMain.run();
	}
