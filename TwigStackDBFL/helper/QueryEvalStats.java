...
public class QueryEvalStats {
	...
	public void printToFile(PrintWriter opw){
		...
		double totNodesBefore =0.0, totNodesAfter = 0.0, totSolnNodes =0.0;
		double totSolns =0.0; 
		...
		for (int i = 0; i < Flags.REPEATS; i++) {
			...
			for (int q = 0; q < numQs; q++) {
				...
				if(i==0){
					totNodesBefore +=stat.totNodes_before;
					totNodesAfter  +=stat.totNodes_after;
				...
				opw.append(q + " " +  f.format(stat.matchTime) + " " 
						           ...
						           + new DecimalFormat(",###").format(stat.totNodes_before) + " "
						           + new DecimalFormat(",###").format(stat.totNodes_after) + " "
	...
	public String toString() {
		...
		double totNodes_before =0.0, totNodes_after = 0.0, totSolnNodes =0.0;
		double totSolns =0.0; 
		for (int i = 0; i < Flags.REPEATS; i++) {
			ArrayList<QueryEvalStat> qryEvalStatList = qryEvalStats[i];
			sb.append("Run " + (i + 1) + ":\r\n");
