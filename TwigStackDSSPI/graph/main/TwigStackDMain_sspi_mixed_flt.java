...
import queryEvaluator.TwigStackD_mixed_flt;
 ...
 *
 */
public class TwigStackDMain_sspi_mixed_flt {
	...
	public TwigStackDMain_sspi_mixed_flt(String dataFN, String queryFN) {

		queryFileN = Consts.INDIR + queryFN;
		dataFileN = Consts.INDIR + dataFN;
		stats = new QueryEvalStats(dataFileN, queryFileN, "TwigStackD-SSPI-mixed-FLT");
	}
	...
	private void evaluate() {
		for (int i = 0; i < Flags.REPEATS; i++) {
			for (int Q = 0; Q < queries_tree.size(); Q++) {
				...
				TwigStackD_mixed_flt tsd = new TwigStackD_mixed_flt(query_tree, invLsts_reduced, SSPI, g);
	...
	public static void main(String[] args) {
		...
		TwigStackDMain_sspi_mixed_flt psMain = new TwigStackDMain_sspi_mixed_flt(dataFileN, queryFileN);
		psMain.run();
	}
