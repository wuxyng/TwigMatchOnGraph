...
import queryEvaluator.TwigStackD_mixed;
 ...
 *
 */
public class TwigStackDMain_sspi_mixed {
	...
	public TwigStackDMain_sspi_mixed(String dataFN, String queryFN) {

		queryFileN = Consts.INDIR + queryFN;
		dataFileN = Consts.INDIR + dataFN;
		stats = new QueryEvalStats(dataFileN, queryFileN, "TwigStackD-SSPI-MIXED");
	}
	...
	private void evaluate() {
		for (int i = 0; i < Flags.REPEATS; i++) {
			for (int Q = 0; Q < queries.size(); Q++) {
				...
				System.out.println("\nEvaluating query " + Q + " ...");
				TwigStackD_mixed tsd = new TwigStackD_mixed(query, invLsts, SSPI, G);
	...
	public static void main(String[] args) {
		...
		TwigStackDMain_sspi_mixed psMain = new TwigStackDMain_sspi_mixed(dataFileN, queryFileN);
		psMain.run();
	}
