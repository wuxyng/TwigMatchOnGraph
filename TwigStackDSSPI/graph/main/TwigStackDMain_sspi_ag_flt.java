 ...
 *
 */
public class TwigStackDMain_sspi_ag_flt {
	...
	public TwigStackDMain_sspi_ag_flt(String dataFN, String queryFN) {

		queryFileN = Consts.INDIR + queryFN;
		dataFileN = Consts.INDIR + dataFN;
		stats = new QueryEvalStats(dataFileN, queryFileN, "TwigStackD-SSPI-MJ-FLT");
	}
	...
	private void evaluate() {
		for (int i = 0; i < Flags.REPEATS; i++) {
			for (int Q = 0; Q < queries_tree.size(); Q++) {
				...
				TwigStackD_ag_flt tsd = new TwigStackD_ag_flt(query_tree, invLsts_reduced, SSPI);
	...
	public static void main(String[] args) {
		...
		TwigStackDMain_sspi_ag_flt psMain = new TwigStackDMain_sspi_ag_flt(dataFileN, queryFileN);
		psMain.run();
	}
