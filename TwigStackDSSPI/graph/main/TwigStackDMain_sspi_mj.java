 ...
 *
 */
public class TwigStackDMain_sspi_mj {
	...
	public TwigStackDMain_sspi_mj(String dataFN, String queryFN) {

		queryFileN = Consts.INDIR + queryFN;
		dataFileN = Consts.INDIR + dataFN;
		stats = new QueryEvalStats(dataFileN, queryFileN, "TwigStackD-SSPI-MJ");
	}
	...
	private void evaluate() {
		for (int i = 0; i < Flags.REPEATS; i++) {
			for (int Q = 0; Q < queries.size(); Q++) {
				...
				System.out.println("\nEvaluating query " + Q + " ...");
				TwigStackD_mj tsd = new TwigStackD_mj(query, invLsts, SSPI);
	...
	public static void main(String[] args) {
		...
		TwigStackDMain_sspi_mj psMain = new TwigStackDMain_sspi_mj(dataFileN, queryFileN);
		psMain.run();
	}
